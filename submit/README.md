...
5. Client-side ROM hashing under “Rom Checksum Submission” (file never uploaded). **Required** unless the probed repository's `rom_identity.txt` already carries the complete identity — crc32, md5, sha1, sha256, size and filename of the bare image, which is what snesrecomp's new-project wizard writes — in which case it is prefilled and the local hash is an optional re-check. Cart platforms (SNES, GBA, …): one file drop, gated to the platform's extensions; SNES strips a 512-byte copier header first. PSX: two labeled slots — `.cue` (fills `track_counts` + expected Track&nbsp;01 name), then that first BINARY `.bin`. `.iso` / `.chd` are rejected
6. On submit:
